...
public class QuickSelect<T extends Comparable<T>> {
	...
	public T quickSelect(T[] array, int k) {
		...
		quickSelect(array, k, leftIndex, rightIndex);


	}

	public T quickSelect(T[] array, int k, int leftIndex, int rightIndex) {

		int pivotIndex = partition(array, leftIndex, rightIndex);

		if (array[pivotIndex].compareTo(k) = 0) return array[pivotIndex];
		
	}

	public int partition(T[] array, int leftIndex, int rightIndex) {
	
		//escolhendo o pivot pela mediana
		int pivotIndex = (rightIndex - leftIndex) / 2;

		//colocando o pivot na primeira posicao
		Util.swap(array, pivotIndex, 0);
		...
		//colocando o pivot a frente dos elementos que foram trocados
